process TBP_PARSER {
    ...
    def min_depth_value = min_depth ? "--min_depth ${min_depth}" : "--min_depth 10"
    def min_freq_value = min_frequency ? "--min_frequency ${min_frequency}": "--min_frequency 0.1"
    def min_read_support_value = min_read_support ? "--min_read_support ${min_read_support}": "--min_read_support 10"
    def min_cov_value = min_percent_coverage ? "--min_percent_coverage ${min_percent_coverage}": "--min_percent_coverage 100"
    ...
    def rrs_freq = rrs_frequency ? "--rrs_frequency ${rrs_frequency}": "--rrs_frequency 0.1"
    def rrs_read_sup = rrs_read_support ? "--rrs_read_support ${rrs_read_support}": "--rrs_read_support 10"
    def rr1_freq = rr1_frequency ? "--rrl_frequency ${rr1_frequency}": "--rrl_frequency 0.1"
    def rr1_read_sup = rr1_read_support ? "--rrl_read_support ${rr1_read_support}": "--rrl_read_support 10"
    def rpob499_freq = rpob499_frequency ? "--rpob449_frequency ${rpob499_frequency}": "--rpob449_frequency 0.1"
    def etha237_freq = etha237_frequency ? "--etha237_frequency ${etha237_frequency}": "--etha237_frequency 0.1"
