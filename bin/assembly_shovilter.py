...
def extract_coverage_from_header(header: 
    str, logger: Optional[logging.Logger] = None
) -> Optional[float]:
    """
    ...
    """
    # SPAdes: cov_29.8803
    coverage_match = re.search(r'cov[_=](\d+(\.\d+)?)', header)
    if coverage_match:
        return float(coverage_match.group(1))
    # MEGAHIT: multi=23.8788
    coverage_match = re.search(r'multi=(\d+(\.\d+)?)', header)
    if coverage_match:
        return float(coverage_match.group(1))
    # SKESA: Contig_2_53.4039 (coverage after last underscore)
    coverage_match = re.search(r'Contig_\d+_(\d+(\.\d+)?)', header)
    if coverage_match:
        return float(coverage_match.group(1))
    
    if logger:
        logger.warning(f"Could not extract coverage from header: {header}")
    return None
...
def filter_contigs_by_coverage(
    ...
    for record in records:
        coverage = extract_coverage_from_header(record.description, logger)
