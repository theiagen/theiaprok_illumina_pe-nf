...
def parse_tbprofiler(input):
    with open(input,'r') as tsv_file:
        ...
        with open ("MAIN_LINEAGE.txt", 'wt') as main_lineage:
            main_lineage.write(tsv_dict['main_lineage'])
        with open ("SUB_LINEAGE.txt", 'wt') as sublineage:
            sublineage.write(tsv_dict['sub_lineage'])
        
        with open ("DR_TYPE.txt", 'wt') as dr_type:
            dr_type.write(tsv_dict['drtype'])
        with open ("NUM_DR_VARIANTS.txt", 'wt') as num_dr_variants:
            num_dr_variants.write(tsv_dict['num_dr_variants'])
        with open ("NUM_OTHER_VARIANTS.txt", 'wt') as num_other_variants:
            num_other_variants.write(tsv_dict['num_other_variants'])

        with open ("RESISTANCE_GENES.txt", 'wt') as resistance_genes:
        ...
        with open ("MEDIAN_DEPTH.txt", 'wt') as median_depth:
            median_depth.write(tsv_dict['target_median_depth'])
        with open ("PCT_READS_MAPPED.txt", 'wt') as pct_reads_mapped:
